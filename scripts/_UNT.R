...
table(galicia_stocks$commonname)
...
galicia_data <- galicia_timeseries %>%
  ...
  left_join(species_data %>% select(-Vulnerability, -LongevityWild, -Length, -resilience_score), 
            by = "scientificname")

# ---------------------------------------------------------
# Step 6a: Extract Missing Data Before Imputation
# Create a table of missing TCbest or TBbest before replacing NA values
missing_values_table <- galicia_data %>%
  filter(is.na(TCbest) | is.na(TBbest)) %>%
  select(stockid, scientificname, commonname, year, TCbest, TBbest)

# Save missing values to a CSV for review
write_csv(missing_values_table, "galicia_missing_values.csv")

# Print the number of missing records
cat("Number of records with missing TCbest or TBbest:", nrow(missing_values_table), "\n")

# ---------------------------------------------------------
# Step 6b: Data Cleaning and Processing
# Handle NA values and apply resilience adjustments *after reviewing the missing values*
galicia_data <- galicia_data %>%
  mutate(
    TCbest = ifelse(is.na(TCbest), NA, TCbest),  # Keep NA unless further action is needed
...
# Optional: Inspect the summarized data
