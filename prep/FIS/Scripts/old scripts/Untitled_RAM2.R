...
# Check the functions in the FAOSTAT package to see if 'get_fao_data()' is available
ls("package:FAOSTAT")

# ---------------------------------------------------------
# Step 1: Load RAM Legacy and SAUP/FAO Data
ram_data <- tryCatch({
  load_ramlegacy()
}, error = function(e) {
...
# Fetch data using 'getFAO' (likely the correct function)
# Using 'read_fao' to get the dataset for catch data
catch_data <- read_fao(
  domainCode = "Fisheries",   # Domain related to catch data
  datasetCode = "QCL",        # Dataset code for catch
  countryCode = NULL,         # NULL to return data for all countries
  years = c(1961, 2020)       # Define the year range for the data
)

# View the first few rows of the data
head(catch_data)
# View the first few rows of the catch data
head(catch_data)
# Load FAO regions data (using the "geodata" domain)
fao_regions <- get_fao_data(
  domainCode = "geodata",   # Geospatial domain to retrieve region data
  datasetCode = "RS",       # Dataset for regions (RS - regions)
  itemCode = NULL,          # NULL returns all items
  countryCode = NULL,       # NULL returns data for all countries
  elementCode = NULL,       # NULL returns all elements
  yearRange = c(1961, 2020) # You can specify a year range or leave it as NULL
)
...
# ---------------------------------------------------------
# Step 7: Integrate CMSY & RAM Data for B/Bmsy Calculation (¡Prioritize RAM over CMSY!!!!!)
...
galicia_data <- galicia_data %>%
  ...
  mutate(
    b_bmsy = ifelse(!is.na(ram_bmsy), ram_bmsy, cmsy_bbmsy)  
  )
...
library(FAOSTAT)
fao_fisheries <- FAOquery(dataset = "FBS", domain = "Fisheries")
head(fao_fisheries)
summary(fao_fisheries)
