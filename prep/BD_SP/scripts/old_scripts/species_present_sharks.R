...
head()
# --- Paths ---
single_species_path <- "/Users/batume/Documents/R/GAL_git/prep/BD_SP/SHARKS_RAYS_CHIMAERAS.shp"
...
process_species <- function(shp_path) {
  ...
  tryCatch({
    ...
    log_path <- file.path(output_species_log_dir, paste0(species_name, "_presence_log3.csv"))
...
# --- Run for One File ---
result <- process_species(single_species_path)
if (!is.null(result)) {
...
intersection_path <- "/Users/batume/Documents/R/GAL_git/prep/BD_SP/output/intersections/SHARKS_RAYS_CHIMAERAS_intersection.gpkg"

# --- Load the intersection file ---
intersection <- st_read(intersection_path, quiet = TRUE)
...
# ---  Save to CSV ---
write_csv(data.frame(species = species_list),
          "species_present_in_SHARKS_RAYS_CHIMAERAS.csv")
