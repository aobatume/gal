...
# --- Paths ---
species_dir <- "/Users/batume/Documents/R/GAL_git/prep/BD_SP/big_files/marinefish"
buffer_path <- "/Users/batume/Documents/R/GAL_git/MAPS/EEZ_BUFFER/galicia_fromohi.shp"
output_csv <- "/Users/batume/Documents/R/GAL_git/prep/BD_SP/output/biodiversity_scores_nopara_big_files.csv"
...
process_species <- function(shp_path) {
  ...
  tryCatch({
    ...
    output_path <- file.path(output_shp_dir, paste0(species_name, "_intersection.gpkg"))
    ...
    log_path <- file.path(output_species_log_dir, paste0(species_name, "_presence_log.csv"))
...
for (shp_file in shp_files) {
  result <- process_species(shp_file)
  if (!is.null(result)) {
